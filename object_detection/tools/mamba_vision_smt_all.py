...
class SMTBlock(nn.Module):
    ...
    def __init__(self, dim=64, ca_num_heads=4, sa_num_heads=-1, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        # DropPath（也称为 Stochastic Depth，随机深度）是一种正则化技术，用于在训练深度神经网络时防止过拟合
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity() 
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = SMTMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

        #通过小初始值的可学习参数，逐步缩放残差分支，避免梯度爆炸或消失
        self.gamma_1 = 1.0
        self.gamma_2 = 1.0    
        if use_layerscale:
...
class MambaVisionLayer1(nn.Module):
    """
    MambaVision layer"
    """
    ...
    def __init__(self,
        ...
        if conv:
            ...
            # Use BatchNorm2d as the normalization layer for conv blocks (4D tensors)
            self.blocks = nn.ModuleList([
                SMTBlock(dim=dim, ca_num_heads=4, sa_num_heads=-1, mlp_ratio=4., qkv_bias=True) 
                for i in range(depth)
            ])
            self.transformer_block = True
            self.conv=True
            ...
            self.blocks = nn.ModuleList([
                Block(
                    ...
                    drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                    layer_scale=layer_scale
                ) if i not in transformer_blocks else (
                    SMTBlock(dim=dim, ca_num_heads=4, sa_num_heads=-1, mlp_ratio=4., qkv_bias=True)
                    if i % 2 == 1 else 
                    Block(
                        dim=dim,
...
class MambaVisionLayer2(nn.Module):
    """
    MambaVision layer"
    """
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            level1 = MambaVisionLayer1(dim=int(dim * 2 ** i),
                                     depth=depths[i],
            ...
            level2 = MambaVisionLayer2(dim=int(dim * 2 ** i),
                                    depth=depths[i],
                                    num_heads=num_heads[i],
                                    window_size=window_size[i],
                                    mlp_ratio=mlp_ratio,
...
class MM_mamba_vision_smt_all(MambaVision):
    def __init__(self, 
                 dim,
                 in_dim,
                 depths,
